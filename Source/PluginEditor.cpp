    ...
    : AudioProcessorEditor (&p), audioProcessor (p), treeState(vts)
{
    ...
    // Toggle Button
    enable.setColour(juce::ToggleButton::ColourIds::textColourId, juce::Colours::black);
    enable.setButtonText("Set Enabled?");
    enableToggle = std::make_unique<juce::AudioProcessorValueTreeState::ButtonAttachment>(treeState, "enabled", enable);
